...
const hasAccess = (album, user) => {
    return album.ownerId.equals(user.userId) || album.sharedWith.some(id => id.equals(user.userId))
}
...
app.post("/album", verifyToken, async(req, res) => {
    ...
    try{
     ...
     return res.status(201).json({Album: newAlbum})
        ...
        console.log(error);
        return res.status(500).json({Error: "Error while creating the album", error})
    }
})
...
app.patch("/albums/:albumId", verifyToken, async (req, res) => {
    try {
        ...
        if (!album.ownerId.equals(req.user.userId))
            return res.status(403).json({ error: "Only the owner can update this album." });
...
app.get("/albums", verifyToken, async (req, res) => {
    try {
        const albums = await Album.find({ownerId: req.user.userId})
...
app.get("/albums/shared", verifyToken, async(req, res) => {
    try{
        const albums = await Album.find({sharedWith: req.user.userId}).populate("sharedWith", "email")
...
app.post("/albums/:albumId/share", verifyToken, async (req, res) => {
    try {
        ...
        if (!album) return;
        if (!album.ownerId.equals(req.user.userId))
            return res.status(403).json({ error: "Only the owner can share this album." });
        ...
        if (unknownEmails.length > 0)
            return res.status(400).json({ error: "Some users do not exist.", unknownEmails });
...
app.delete("/albums/:albumId", verifyToken, async (req, res) => {
    try {
        const album = await Album.findById(req.params.albumId);
        if (!album) return res.status(400).json({message: "Album does not exist"});
        if (!album.ownerId.equals(req.user.userId))
            return res.status(403).json({ error: "Only the owner can delete this album." });
...
app.put("/albums/:albumId/images/:imageId/favorite", verifyToken, async (req, res) => {
    try {
        ...
        const image = await Image.findOneAndUpdate(
            { _id: req.params.imageId, albumId: req.params.albumId },
...
app.post("/albums/:albumId/images/:imageId/comments", verifyToken, async (req, res) => {
    try {
        const album = await Album.findById(req.params.albumId);
        if (!album) return;
        if (!album.ownerId.equals(req.user.userId))
